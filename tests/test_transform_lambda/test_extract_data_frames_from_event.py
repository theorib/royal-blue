...
def test_dataframe():
    return pd.DataFrame({
        ...
        "favourite_food": ["Fish and chips", "Jacket potato", "Spaghetti bolognese"],
...
class TestExtractDataFramesFromEvent:
    ...
    @pytest.mark.it("Should return a valid dictionary of DataFrames from a valid S3 event")
    def test_returns_dataframe_dict_from_valid_event(self, test_dataframe, test_mock_event):
        buffer = BytesIO()
        test_dataframe.to_parquet(buffer, index=False)
        parquet_bytes = buffer.getvalue()

        with patch("src.lambdas.transform_lambda.utilities.extract_dataframe_from_event.get_file_from_s3_bucket") as mock_get_file_from_s3:
    ...
    def test_raises_exception_on_invalid_parquet_data(self, test_mock_event):
        with patch("src.lambdas.transform_lambda.utilities.extract_dataframe_from_event.get_file_from_s3_bucket") as mock_get_file_from_s3:
            mock_get_file_from_s3.return_value = {"success": {"data": b"not-a-parquet-file"}}
    ...
    def test_raises_exception_on_s3_error_response(self, test_mock_event):
        with patch("src.lambdas.transform_lambda.utilities.extract_dataframe_from_event.get_file_from_s3_bucket") as mock_get_file_from_s3:
            mock_get_file_from_s3.return_value = {"error": {"message": "File not found"}}
