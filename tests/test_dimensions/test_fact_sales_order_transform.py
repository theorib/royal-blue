...
from src.lambdas.dimensions.fact_sales_order_transform import fact_sales_order_dataframe
...
class TestFactSalesOrder:    
    @pytest.mark.it("Should return a DataFrame with the required columns when input is valid")
    ...
    @pytest.mark.it("Should raise ValueError if 'sales_order' table is missing from input")
    def test_fact_sales_order_missing_table(self):
        df = {}

        with pytest.raises(ValueError, match="Missing 'sales_order' table from extracted data."):
            fact_sales_order_dataframe(df)

    @pytest.mark.it("Should raise ValueError if required columns are missing from 'sales_order'")
