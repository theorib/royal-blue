...
def test_dim_design_empty_DF(mock_design_df):
    empty_df = pd.DataFrame(columns=["design_id", "design_name", "file_location", "file_name"])
    dfs = {"design": empty_df}
...
def test_dim_design_duplicates(mock_design_df):
    duplicated_df = pd.concat([mock_design_df, mock_design_df.iloc[[0]]], ignore_index=True)
