...
from lambdas.transform_lambda.utils.convert_parquet_to_dataframe import (
...
def extract_dataframes_from_event(client, event):
    ...
    try:
        for table in event:
            key = table['key']  
            parquet_file = get_file_from_s3_bucket(client, BUCKET_NAME, key)
            data_frame = parquet_to_dataframe(parquet_file)
